...
import { BaseUrl, ContactPage, LoginPage } from "../../utils";
import { Element_Contact, ensureSearchPanelOpen } from "./Elemenet_Contact";
import { FillInputContactForm, ContactFormFields } from "./FillForm";
import { performLogin, performReLogin } from "../../auth_utils";

test.afterEach(async ({ page, context }, testInfo) => {
  // Check if test failed
  if (testInfo.status !== 'passed') {
    const url = page.url();
    // Check if on login page OR network/timeout errors
    if (
      url.includes('/login') ||
      url.includes('Login') ||
      url === LoginPage ||
      testInfo.error?.message?.includes('Network') ||
      testInfo.error?.message?.includes('fetch') ||
      testInfo.error?.message?.includes('Target page, context or browser has been closed')
    ) {
      console.log(`Test failed. Current URL: ${url}. Triggering re-login sequence to refresh session...`);
      // Remove framenavigated listener to prevent it from triggering during re-login
      page.removeAllListeners('framenavigated');

      // Extend timeout to allow re-login to complete (adding 60 seconds)
      testInfo.setTimeout(testInfo.timeout + 60000);

      // Perform re-login which handles saving the storage state
      await performReLogin(page);
      console.log('Session refreshed. Test will be retried if retries are enabled.');
    }
  }
  await page.close();
  await context.close();
...
// Generate a random 5-digit number for unique names (Total length <= 10 chars)
const randomSuffix = Math.floor(10000 + Math.random() * 90000); // Ensures 5 digits
const contactData = {
  Name: `Test${randomSuffix}`, // e.g., Test12345 (9 chars)
  ...
  Change_name: `Edit${randomSuffix}`, // e.g., Edit12345 (9 chars)
  Change_phone: '0111111',
  Change_email: 'edit_123@test.com',
...
const contact_Search_Data = {
  ...
  Address_province1_Edit: "นนทบุรี",
  Address_subdistrict: "บางแวก",
  Address_subdistrict1_Edit: "หนองบอน",
  Address_district: "บางกอกน้อย",
  Address_district1_Edit: "บางแค",
...
test.beforeEach(async ({ page }) => {
  ...
  // // Fail fast if redirected to Login page
  // page.on('framenavigated', async (frame) => {
  //   if (frame === page.mainFrame()) {
  //     const url = frame.url();
  //     if (url.includes('/login') || url.includes('Login') || url === LoginPage) {
  //       console.log('Redirected to Login page detected. Failing test immediately to trigger re-login...');
...
test.describe('Contact Search Tests', () => {
  ...
  test('CRM_CT00002 การเข้าหน้าค้นหาลูกค้า Search', async ({ page }) => {
    ...
    await contact.ensureSearchPanelOpen();
  ...
  test('CRM_CT00009   "การค้นหาช่องใส่ Phone กรณีกำหนดให้ Integer พิมพ์ได้เฉพาะตัวเลขเท่านั้น"', async ({ page }) => {
    ...
    await page.getByText('Invalid numeric format').waitFor();
    // Trigger validation
    await expect(page.getByText('Invalid numeric format')).toBeVisible();
  ...
  test('CRM_CT00012   "การค้นหาช่องใส่ Email กรณีกรอกรูปแบบอีเมลไม่ถูกต้อง"', async ({ page, request }) => {
    ...
    await page.getByText('Invalid email format').waitFor();
    // Trigger validation
  ...
  test('CRM_CT00026	"การค้นหาช่องเลือก Collection Level 1 กรณีมีรายชื่อลูกค้าไม่มีอยู่ในระบบ""', async ({ page }) => {
    const contact = new Element_Contact(page);
    await contact.goto();
    await contact.searchByMultipleDropdown({ MultipleDropdownlv1: 'Level1-3' }); // Assuming this option exists but no data
    await contact.expectNoData();
  });
  ...
  test.describe('Contact Create Tests', () => {
    ...
    test('CRM_CT00056	"การใส่ข้อมูล ช่องName กรณีใส่Nameซ้ำ"" ', async ({ page }) => {
      ...
      // Wait for at least 3 rows (more than 2)
      const rows = page.locator('#dyn_contactTable tr[id^="dyn_rows_"]');
      await rows.nth(2).waitFor();

      // Find a name with length <= 10
      let targetName = '';
      const count = await rows.count();

      for (let i = 0; i < count; i++) {
        const name = await rows.nth(i).locator('#dyn_row_name').textContent();
        if (name && name.trim().length <= 10) {
          targetName = name.trim();
          console.log(`Found target duplicate name: ${targetName}`);
          break;
        }
      }

      if (!targetName) {
        console.log('No suitable name found (<= 10 chars). Using fallback.');
        targetName = 'Duplicate'; // Fallback if no suitable name found
      }

      await contact.createContact()

      // Fill required fields with data that includes the duplicate name
      const duplicateData = { ...contactData, Name: targetName };
      await contact.input_Field(duplicateData);

      await contact.submmit_contact.click();
      await contact.btnconfirm_create.click()

      const errorMsg = page.getByText('Name already exists').or(page.getByText('มีข้อมูลนี้อยู่แล้ว'));
    ...
    test('CRM_CT00065	"การใส่ข้อมูลช่อง Address ที่อยู่ " ', async ({ page }) => {
      ...
      await expect(contact.input_address).toHaveValue('ทดสอบการใส่ที่อยู')
    ...
    test('CRM_CT00066	กรณีค้นหา ตำบล/แขวง ต้องใส่ตัวอักษรที่อยู่ในdropdown ข้อมูลสถานที่ถึงแสดง" ', async ({ page }) => {
      ...
      await contact.btn_address.click();
      await page.locator('.grid.grid-cols-2 > div:nth-child(2) > #dropdownEl > .relative > .w-8').waitFor()
      ...
      // Retry logic: if option not found, press backspace to trigger search
      try {
        await page.getByRole('option', { name: 'บางแวก » ภาษีเจริญ » กรุงเทพมหานคร »' }).waitFor({ state: 'visible', timeout: 3000 });
      } catch (e) {
        console.log('Option not found, pressing Backspace to trigger search...');
        await page.getByRole('combobox', { name: 'ค้นหา ตำบล/แขวง' }).press('Backspace');
        await page.getByRole('option', { name: 'บางแวก » ภาษีเจริญ » กรุงเทพมหานคร »' }).waitFor({ state: 'visible', timeout: 5000 });
      }

      await page.getByRole('option', { name: 'บางแวก » ภาษีเจริญ » กรุงเทพมหานคร »' }).click();


      await expect(page.getByRole('combobox', { name: 'บางแวก' })).toBeVisible()
    ...
    test('CRM_CT00067	กรณีค้นหา อำเภอ/เขต ต้องใส่ตัวอักษรที่อยู่ในdropdown ข้อมูลสถานที่ถึงแสดง" ', async ({ page }) => {
      ...
      await contact.createContact()
      await contact.btn_address.click();
      await page.locator('div:nth-child(3) > #dropdownEl > .relative > .w-8 > .lucide').waitFor()
      await page.locator('div:nth-child(3) > #dropdownEl > .relative > .w-8 > .lucide').click()
      await page.getByRole('combobox', { name: 'ค้นหา อำเภอ / เขต' }).waitFor()
      await page.getByRole('combobox', { name: 'ค้นหา อำเภอ / เขต' }).fill(contact_Search_Data.Address_district);

      await page.getByText('บางขุนนนท์ » บางกอกน้อย » กรุงเทพมหานคร »').waitFor()
      await page.getByText('บางขุนนนท์ » บางกอกน้อย » กรุงเทพมหานคร »').click()
      await expect(page.getByRole('combobox', { name: contactData.Address_district1 })).toBeVisible()
    });
    test('CRM_CT00068	กรณีค้นหา จังหวัด ต้องใส่ตัวอักษรที่อยู่ในdropdown ข้อมูลสถานที่ถึงแสดง" ', async ({ page }) => {
      ...
      await contact.createContact()
      await contact.btn_address.click();
      await page.locator('.grid.grid-cols-2 > div:nth-child(4) > #dropdownEl > .relative > .w-8').waitFor()
      await page.locator('.grid.grid-cols-2 > div:nth-child(4) > #dropdownEl > .relative > .w-8').click()
      // await page.waitForTimeout(1000)
      await page.getByRole('combobox', { name: 'ค้นหา จังหวัด' }).waitFor()
    ...
    test('CRM_CT00069	กรณีค้นหา รหัสไปรษณีย์ ต้องใส่ตัวอักษรที่อยู่ในdropdown ข้อมูลสถานที่ถึงแสดง" ', async ({ page }) => {
      ...
      // await page.waitForTimeout(1000)
      await page.locator('.grid.grid-cols-2 > div:nth-child(5) > #dropdownEl > .relative > .w-8 > .lucide').waitFor()
      ...
      await expect(page.getByRole('combobox', { name: '10160' })).toBeVisible()
    });
    test('CRM_CT00070	"การใส่ข้อมูลช่อง Address กรณีไม่ใส่ข้อมูลที่อยู่ "" ', async ({ page }) => {
      ...
      await expect(page.getByText('Value is required').nth(1)).toBeVisible();
      await expect(page.getByText('Value is required').nth(2)).toBeVisible();
      await expect(page.getByText('Value is required').nth(3)).toBeVisible();
      await expect(page.getByText('Value is required').nth(4)).toBeVisible();
    });
    test('CRM_CT00071	การเลือกข้อมูลช่อง Dropdown"" ', async ({ page }) => {
      ...
      await expect(page.getByRole('combobox', { name: 'ทดสอบตัวเลือก' })).toBeVisible()
    ...
    test('CRM_CT00072	การเลือกข้อมูลช่อง Multi Dropdown"" ', async ({ page }) => {
      ...
      await expect(page.getByRole('combobox', { name: multipleDropdownData.MultipleDropdownlv1 })).toBeVisible();
      await expect(page.getByRole('combobox', { name: multipleDropdownData.MultipleDropdownlv2 })).toBeVisible();
      await expect(page.getByRole('combobox', { name: multipleDropdownData.MultipleDropdownlv3 })).toBeVisible();
      await expect(page.getByRole('combobox', { name: multipleDropdownData.MultipleDropdownlv4 })).toBeVisible();
      await expect(page.getByRole('combobox', { name: multipleDropdownData.MultipleDropdownlv5 })).toBeVisible();
      await expect(page.getByRole('combobox', { name: multipleDropdownData.MultipleDropdownlv6 })).toBeVisible();
    });
    ...
    test('CRM_CT00074	"การใส่ข้อมูลช่อง Text Input กรณีใส่ Text Input ความยาวตัวอักษร สูงสุด 10 The maximum length allowed is 10"" ', async ({ page }) => {
      ...
      await expect(page.getByText('Name *The maximum length')).toBeVisible()
    ...
    test('CRM_CT00075	"การใส่ข้อมูลช่อง Text Input กรณีีไม่ใส่ Text Input Value is required""" ', async ({ page }) => {
      ...
      await contact.createContact()
      await page.getByRole('button', { name: 'Create', exact: true }).click();
      await expect(page.getByText('Name *Value is required')).toBeVisible()
    ...
    test('CRM_CT00084	"การเลือกกลุ่มและข้อมูลในกลุ่มที่เลือก จะแสดงก็ต่อเมื่อมีการเปลี่ยนแปลง Segment" " ', async ({ page }) => {
      ...
      await expect(page.locator('#dyn_text_segment')).toBeVisible()
    ...
    test('CRM_CT00091	"การสร้างเนื้อหา อัปโหลด Attach File (Type DOC) กรณีขนาดไฟล์เกิน5MB" "" ', async ({ page }) => {
      ...
      // await page.waitForTimeout(1000)
      await contact.error_attach_file.waitFor();
  ...
  test.describe.serial(' Edit Tests', () => {
    ...
    test('CRM_CT00111	การส่งออกข้อมูลลูกค้า (ปุ่มExport)', async ({ page }) => {
      ...
      await expect(page.getByText('Report export successful.')).toBeVisible()
    ...
    test('CRM_CT00115	การเข้าชมข้อมูลลูกค้า (View Contact) ========== @smoke', async ({ page }) => {
      ...
      const contact = new Element_Create_Contact(page);
      await contact.goto();
      await verifyTopTableRow(page, { CheckView: currentContactName })


      await expect(contact.inputName.inputValue()).toBe(currentContactName);
      ...
      await expect(contact.inputEmail.inputValue()).toBe(contactData.Email);


      const checkboxHTML = await contact.inputCheckbox.evaluate(el => el.outerHTML);
      console.log(`DEBUG: Checkbox outerHTML: ${checkboxHTML}`);

      const radioHTML = await contact.btnRadio.evaluate(el => el.outerHTML);
      console.log(`DEBUG: Radio outerHTML: ${radioHTML}`);
      if (contactData.Checkbox === true) {
        await expect(contact.inputCheckbox).toBeChecked();
      } else {
        await expect(contact.inputCheckbox).not.toBeChecked();
      }

      // Verify Radio button by ID mapping as requested
      let radioId = '';
      if (contactData.Radiobtn === 'value1') {
        radioId = '#dyn_radiobtn_0';
      } else if (contactData.Radiobtn === 'value2') {
        radioId = '#dyn_radiobtn_1';
      } else if (contactData.Radiobtn === 'value3') {
        radioId = '#dyn_radiobtn_2';
      }

      if (radioId) {
        // Check if the specific radio button is checked
        await expect(page.locator(radioId)).toBeChecked();
      } else {
        console.warn(`Unexpected Radiobtn value: ${contactData.Radiobtn}`);
      }

      await expect(contact.input_Create_DateTime.inputValue()).toBe(contactData.DateTime);

      await expect(contact.input_Create_DateTime.inputValue()).toBe(contactData.DateTime);
      await expect(contact.input_Create_Date.inputValue()).toBe(contactData.Date);
      await expect(contact.input_Create_Time.inputValue()).toBe(contactData.Time);
      await expect(contact.segment.inputValue()).toBe(contactData.Segment);
      await expect(contact.input_segment.inputValue()).toBe(contactData.Input_Segment);
      await expect(contact.addressNo.inputValue()).toBe(contactData.Address_no1);
    ...
    test('CRM_CT00116	การแก้ไขข้อมูลลูกค้า (Edit Contact)', async ({ page }) => {
      ...
      const contact = new Element_Create_Contact(page);
      await contact.goto();
      await verifyTopTableRow(page, { CheckEdit: currentContactName })
      ...
      await verifyTopTableRow(page, { Name: currentContactName })
    ...
    test('CRM_CT00117	การแก้ไขช่องใส่ Name @smoke', async ({ page }) => {
      ...
      // await page.waitForTimeout(3000)
      await verifyTopTableRow(page, { Name: currentContactName })
      currentContactName = contactData.Change_name;
    ...
    test('CRM_CT00118	"การแก้ไขช่องใส่ Name กรณีใส่Nameซ้ำ"', async ({ page }) => {
      ...
      // Find a name with length <= 10 (Target Duplicate)
      let targetName = '';
      const rows = page.locator('#dyn_contactTable tr[id^="dyn_rows_"]');
      await rows.nth(2).waitFor(); // Wait for at least 3 rows
      const count = await rows.count();
      ...
      if (!targetName) {
        console.log('No suitable name found (<= 10 chars). Using fallback.');
        targetName = 'Duplicate'; // Fallback
      }

      // 3. Get Name from 1st row (to be edited) - Ensure it's NOT the same as targetName
      // We can just pick the first row that isn't the targetName
      let firstRowName = '';
      for (let i = 0; i < count; i++) {
        const name = await rows.nth(i).locator('#dyn_row_name').textContent();
        if (name && name.trim() !== targetName) {
          firstRowName = name.trim();
          console.log('Name from row (To be edited):', firstRowName);
          break;
        }
      }

      // 4. Edit the selected row
      await verifyTopTableRow(page, { CheckEdit: firstRowName });

      // 5. Try to change name to match the target duplicate name
      await contact.input_Field({ Name: targetName });
      await contact.btnUpdate.click();
      await contact.btnconfirm_update.click();
    ...
    test('CRM_CT00124 "การแก้ไขช่องใส่ Email"', async ({ page }) => {
      ...
      await contact.goto();
      await verifyTopTableRow(page, { CheckEdit: currentContactName });
      await contact.inputEmail.fill(contactData.Change_email);
      await contact.btnUpdate.click();
      await contact.btnconfirm_update.click();
      // await page.waitForTimeout(3000);
      await verifyTopTableRow(page, { Email: contactData.Change_email });
    });
    ...
    test('CRM_CT00134 "การเลือกข้อมูลช่อง Multi Dropdown"', async ({ page }) => {
      ...
      await page.locator('#dyn_JEFOkL_clear > .lucide').click();
      // await page.waitForTimeout(500);
    ...
    test('CRM_CT00150 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type PDF)"ขนาดไฟล์ไม่เกิน5MB', async ({ page }) => {
      const contact = new Element_Create_Contact(page);
      await contact.goto();
      await verifyTopTableRow(page, { CheckEdit: currentContactName });

      await page.waitForTimeout(3000);
      // เช็คจำนวนไฟล์ 'test-pdf.pdf' ก่อนอัปโหลด
      await page.waitForTimeout(3000);
      const initialFiles = await page.locator('.filepond--file-info-main').allTextContents();
      const initialTargetCount = initialFiles.filter(name => name === 'test-pdf.pdf').length;
      console.log(`Initial count of 'test-pdf.pdf': ${initialTargetCount}`);

      // รับค่า removedFileName จาก function
      const uploadResult = await uploadFileWithLimitCheck(page, 'tests/file_update-test/test-pdf.pdf');
      const removedFileName = uploadResult.removedFileName;
      ...
      // ตรวจสอบว่าจำนวนไฟล์ 'test-pdf.pdf' ต้องเพิ่มขึ้นตาม logic
      const finalTargetCount = uploadedFileNames.filter(name => name === 'test-pdf.pdf').length;
      console.log(`Final count of 'test-pdf.pdf': ${finalTargetCount}`);

      // Logic: ถ้าไฟล์ที่ถูกลบไป คือไฟล์เดียวกับที่อัปโหลด (test-pdf.pdf) => จำนวนรวมต้องเท่าเดิม (ลบ 1 เพิ่ม 1)
      // ถ้าไฟล์ที่ถูกลบไป ไม่ใช่ test-pdf.pdf หรือไม่มีการลบ => จำนวนรวมต้องเพิ่ม 1
      let expectedIncrement = 1;
      if (removedFileName === 'test-pdf.pdf') {
        console.log('Detected that the removed file matches the uploaded file. Expecting count to remain same.');
        expectedIncrement = 0;
      }

      expect(finalTargetCount).toBe(initialTargetCount + expectedIncrement);
      expect(uploadedFileNames).toContain('test-pdf.pdf');
    });
    ...
    test('CRM_CT00152 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type DOC) ""ขนาดไฟล์ไม่เกิน5MB"" "', async ({ page }) => {
      ...
      const fileName = 'doc-test-edit.doc';
      // เช็คจำนวนไฟล์ก่อนอัปโหลด
      await page.waitForTimeout(3000);
      const initialFiles = await page.locator('.filepond--file-info-main').allTextContents();
      const initialTargetCount = initialFiles.filter(name => name === fileName).length;
      console.log(`Initial count of '${fileName}': ${initialTargetCount}`);

      // รับค่า removedFileName จาก function
      const uploadResult = await uploadFileWithLimitCheck(page, `tests/file_update-test/${fileName}`);
      const removedFileName = uploadResult.removedFileName;
      ...
      // เข้ามาตรวจสอบในรายการว่ามีไฟล์
      await verifyTopTableRow(page, { CheckEdit: currentContactName });

      await page.waitForTimeout(5000);
      const uploadedFileNames = await page.locator('.filepond--file-info-main').allTextContents();

      // ตรวจสอบว่าจำนวนไฟล์ต้องเพิ่มขึ้นตาม logic
      const finalTargetCount = uploadedFileNames.filter(name => name === fileName).length;
      console.log(`Final count of '${fileName}': ${finalTargetCount}`);

      let expectedIncrement = 1;
      if (removedFileName === fileName) {
        console.log('Detected that the removed file matches the uploaded file. Expecting count to remain same.');
        expectedIncrement = 0;
      }
    ...
    test('CRM_CT00154 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type DOCX) ""ขนาดไฟล์ไม่เกิน5MB"" "', async ({ page }) => {
      ...
      const fileName = 'docx-test-edit.docx';
      // เช็คจำนวนไฟล์ก่อนอัปโหลด
      await page.waitForTimeout(3000);
      const initialFiles = await page.locator('.filepond--file-info-main').allTextContents();
      const initialTargetCount = initialFiles.filter(name => name === fileName).length;
    ...
    test('CRM_CT00160 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type CSV) ""ขนาดไฟล์ไม่เกิน5MB"" "', async ({ page }) => {
      ...
      const fileName = 'csv-test-edit.csv';
      // เช็คจำนวนไฟล์ก่อนอัปโหลด
      await page.waitForTimeout(3000);
      const initialFiles = await page.locator('.filepond--file-info-main').allTextContents();
      const initialTargetCount = initialFiles.filter(name => name === fileName).length;
    ...
    test('CRM_CT00162 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type PNG) ""ขนาดไฟล์ไม่เกิน5MB"" "', async ({ page }) => {
      ...
      await uploadFileWithLimitCheck(page, 'tests/file_update-test/png.png');
      ...
      await page.waitForTimeout(5000);
      const uploadedFileNames = await page.locator('.filepond--file-info-main').allTextContents();
      expect(uploadedFileNames).toContain('png.png');
    });
    ...
    test('CRM_CT00164 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type JPG) ""ขนาดไฟล์ไม่เกิน5MB"" "', async ({ page }) => {
      ...
      await uploadFileWithLimitCheck(page, 'tests/file_update-test/jpg-test.jpg');
      ...
      await page.waitForTimeout(5000);
      const uploadedFileNames = await page.locator('.filepond--file-info-main').allTextContents();
      expect(uploadedFileNames).toContain('jpg-test.jpg');
    });
    ...
    test('CRM_CT00166 "การแก้ไขเนื้อหา อัปโหลด Attach File (Type JPEG) ""ขนาดไฟล์ไม่เกิน5MB"" "', async ({ page }) => {
      ...
      await uploadFileWithLimitCheck(page, 'tests/file_update-test/small.jpeg');
      ...
      await page.waitForTimeout(5000);
      const uploadedFileNames = await page.locator('.filepond--file-info-main').allTextContents();
      expect(uploadedFileNames).toContain('small.jpeg');
    });
    ...
    test('CRM_CT00170 "การจดบันทึก Note ======= แก้ไข"', async ({ page }) => {
      ...
      const noteText = `Test Note ${timestamp} ${formatTime(now)}`;
